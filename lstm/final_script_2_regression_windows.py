...
df = df[['PESPANIA'] + [x for x in cols if x.startswith('L') and 'PESPANIA' in x] +
        ['CONTROL', 'DIA', 'TARGET']]
...
train_normal_x = np.reshape(train_normal_x, (train_normal_x.shape[0], look_back, train_normal_x.shape[1])) # TERCERO: TENEMOS QUE DEFINIR SAMPLES, TIME_STEPS=LAGS, FEATURES=FEATURES
valid_normal_x = np.reshape(valid_normal_x, (valid_normal_x.shape[0], look_back, valid_normal_x.shape[1]))
valid_mixed_x = np.reshape(valid_mixed_x, (valid_mixed_x.shape[0], look_back,valid_mixed_x.shape[1]))
...
# LSTM
model = Sequential()
model.add(LSTM(50, input_shape=(1, lags), return_sequences=True, activation='tanh'))
model.add(Dropout(0.2))
model.add(LSTM(50, return_sequences=False, activation='tanh'))
model.add(Dropout(0.2))
model.add(Dense(1, activation='linear'))
model.compile(loss=loss.mean_absolute_error, optimizer='adam')
...
train_true = scaler.inverse_transform([train_normal_y])
...
valid2_true = scaler.inverse_transform([valid_mixed_y])
valid2_true = valid2_true[0]
